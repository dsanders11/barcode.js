...
define(['chai'], function(chai) {
  ...
  describe('ReedSolomon', function() {
    it('testDataMatrix', function() {
      // Long running test
      this.timeout(5000);

      // real life test cases
      testEncodeDecode(GenericGF.DATA_MATRIX_FIELD_256,
          Int32Array.of(142, 164, 186), Int32Array.of(114, 25, 5, 88, 102));
      testEncodeDecode(GenericGF.DATA_MATRIX_FIELD_256, Int32Array.of(
    ...
    it('testQRCode', function() {
      // Long running test
      this.timeout(5000);

      // Test case from example given in ISO 18004, Annex I
      testEncodeDecode(GenericGF.QR_CODE_FIELD_256, Int32Array.of(
    ...
    it('testAztec', function() {
      // Long running test
      this.timeout(5000);

      // real life test cases
      testEncodeDecode(GenericGF.AZTEC_PARAM,
          Int32Array.of(0x5, 0x6), Int32Array.of(0x3, 0x2, 0xB, 0xB, 0x7));
      testEncodeDecode(GenericGF.AZTEC_PARAM,
          Int32Array.of(0x0, 0x0, 0x0, 0x9), Int32Array.of(0xA, 0xD, 0x8, 0x6, 0x5, 0x6));
      testEncodeDecode(GenericGF.AZTEC_PARAM,
          Int32Array.of(0x2, 0x8, 0x8, 0x7), Int32Array.of(0xE, 0xC, 0xA, 0x9, 0x6, 0x8));
      testEncodeDecode(GenericGF.AZTEC_DATA_6, Int32Array.of(
          0x9, 0x32, 0x1, 0x29, 0x2F, 0x2, 0x27, 0x25, 0x1, 0x1B),
          Int32Array.of(
          0x2C, 0x2, 0xD, 0xD, 0xA, 0x16, 0x28, 0x9, 0x22, 0xA, 0x14));
      testEncodeDecode(GenericGF.AZTEC_DATA_8, Int32Array.of(
          ...
          0x9F, 0xC1, 0x75));
      testEncodeDecode(GenericGF.AZTEC_DATA_10, Int32Array.of(
      ...
      testEncodeDecode(GenericGF.AZTEC_DATA_12, Int32Array.of(
    ...
    function corrupt(received, howMany, random, max) {
      ...
      for (let j = 0; j < howMany; j++) {
        let location = random.nextInt(received.length);
        let value = random.nextInt(max);
    ...
    function testEncodeDecodeRandom(field, dataSize, ecSize) {
      assert.isTrue(dataSize > 0 && dataSize <= field.getSize() - 3, "Invalid data size for " + field);
      assert.isTrue(ecSize > 0 && ecSize + dataSize <= field.getSize(), "Invalid ECC size for " + field);
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        // generate ECC words
        message.set(dataWords);
        encoder.encode(message, ecWords.length);
        ecWords.set(message.slice(dataSize, dataSize + ecSize));
    ...
    function testDecoder(field, dataWords, ecWords) {
      ...
      var message = new Int32Array(dataWords.length + ecWords.length);
      var maxErrors = ecWords.length >> 1;
      var random = getPseudoRandom();
      ...
      for (let j = 0; j < iterations; j++) {
        for (let i = 0; i < ecWords.length; i++) {
          ...
          message.set(dataWords);
          message.set(ecWords, dataWords.length);
          corrupt(message, i, random, field.getSize());
          try {
            decoder.decode(message, ecWords.length);
          } catch (err) {
            // fail only if maxErrors exceeded
            assert.isTrue(i > maxErrors, "Decode in " + field + " (" + dataWords.length + ',' + ecWords.length + ") failed at " +
                           i + " errors: " + err);
            // else stop
            break;
          }
    ...
    function assertDataEquals(message, expected, received) {
      for (let i = 0; i < expected.length; i++) {
        if (expected[i] != received[i]) {
          assert.fail(arrayToString(received.slice(0, expected.length)), arrayToString(expected));
        }
      }
    }
